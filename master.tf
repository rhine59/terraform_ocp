resource "vsphere_virtual_machine" "master" {
  connection {
    ...
    user = "root"
    password = "Passw0rd="
    host = "${cidrhost(var.cluster_network, var.master_ip_start + count.index)}"  
  }
  count            = "${var.master_count}"
  name 		   = "${var.cluster_prefix}mstr${format("%01d",  count.index + 1)}"
  ...
  clone {
    ...
    customize {
      linux_options {
  	host_name = "${var.cluster_prefix}mstr${format("%01d",  count.index + 1)}"
        domain    = var.virtual_machine_domain
      }
      network_interface {
	ipv4_address = "${cidrhost(var.cluster_network, var.master_ip_start + count.index)}"  
  ...
  provisioner "file" {
    content = <<EOF
#!/bin/bash
curl -X POST -H 'Content-type: application/json' --data '{"text":"Master starting!"}' https://hooks.slack.com/services/${var.slack_channel} >/dev/null 2>&1
