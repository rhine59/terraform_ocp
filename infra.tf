resource "vsphere_virtual_machine" "infra" {
  connection {
    ...
    user = "root"
    password = "Passw0rd="
    host = "${cidrhost(var.cluster_network, var.infra_ip_start + count.index)}"  
  }
  count            = "${var.infra_count}"
  name 		   = "${var.cluster_prefix}infra${format("%01d",  count.index + 1)}"
  ...
  clone {
    ...
    customize {
      linux_options {
  	host_name = "${var.cluster_prefix}infra${format("%01d",  count.index + 1)}"
        domain    = var.virtual_machine_domain
      }
      network_interface {
	ipv4_address = "${cidrhost(var.cluster_network, var.infra_ip_start + count.index)}"  
  ...
  provisioner "file" {
    content = <<EOF
#!/bin/bash
echo infra ${var.cluster_prefix}infra${format("%01d",  count.index + 1)} ${cidrhost(var.cluster_network, var.infra_ip_start + count.index)} > /tmp/inventory
curl -X POST -H 'Content-type: application/json' --data '{"text":"Infra Starting!"}' https://hooks.slack.com/services/${var.slack_channel} >/dev/null 2>&1
...
# We will use disk3, /dev/sdd as our gluster volume so needs to be left as a raw device
